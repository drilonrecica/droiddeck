...
export function parseLogLine(raw: string): LogLine {
  const threadtime =
    /^(\d\d-\d\d\s+\d\d:\d\d:\d\d\.\d+)\s+(\d+)\s+(\d+)\s+([VDIWEF])\s+([^:]+):\s?(.*)$/.exec(raw);
  if (threadtime) {
    return {
...
export function shouldShowLogLine(line: LogLine, mode: LogMode): boolean {
  if (mode === "all") {
    return true;
  }
  if (!line.priority) {
  ...
  return priorityRank[line.priority] >= (mode === "errors" ? priorityRank.E : priorityRank.W);
}
